    ...
    <div class="grid-2">
        ...
        <div class="card">
            ...
            <div style="padding: 20px;">
                <!-- Status Badge -->
                <div class="mb-6 text-center" style="padding: 24px; background: var(--bg-tertiary); border-radius: var(--radius-md);">
                    <div class="badge badge-{{ $attendance->status_color }}" style="font-size: 18px; padding: 12px 32px;">
                ...
                <div class="info-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div>
                        <label class="text-muted" style="font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">Tanggal</label>
                    ...
                    <div>
                        <label class="text-muted" style="font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">Siswa</label>
                        <div class="fw-bold" style="margin-top: 4px;">
                            @if($attendance->intern)
                ...
                <div class="mt-6" style="padding: 16px; background: var(--bg-tertiary); border-radius: var(--radius-md); text-align: center;">
                    <label class="text-muted" style="font-size: 12px; text-transform: uppercase;">Total Jam Kerja</label>
        ...
        <div class="card">
            ...
            <div style="padding: 20px;">
                ...
                <div class="mb-6">
                    <label class="text-muted" style="font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">
                        <i class="fas fa-exclamation-triangle text-warning"></i> Alasan Keterlambatan
                    </label>
                    <div style="margin-top: 8px; padding: 16px; background: rgba(234, 179, 8, 0.1); border: 1px solid rgba(234, 179, 8, 0.3); border-radius: var(--radius-md);">
                        @if($attendance->late_reason)
                            <p style="margin: 0;">{{ $attendance->late_reason }}</p>
                        @else
                            <p class="text-muted" style="margin: 0; font-style: italic;">Tidak ada alasan yang diberikan</p>
                        @endif
                    </div>
                </div>
                @endif
                ...
                <div class="mb-6">
                    <label class="text-muted" style="font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">Catatan</label>
                    <div style="margin-top: 8px; padding: 16px; background: var(--bg-tertiary); border-radius: var(--radius-md);">
                        @if($attendance->notes)
                ...
                <div style="padding: 16px; background: var(--bg-tertiary); border-radius: var(--radius-md);">
                    <label class="text-muted" style="font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; display: block;">Info Siswa</label>
                    <div class="d-flex align-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="text-muted" style="font-size: 13px;">{{ $attendance->intern->school }}</div>
                        </div>
                    </div>
                </div>
                @endif
    ...
    <div class="card mt-6">
        ...
        <div style="padding: 20px;">
            @if($attendance->latitude && $attendance->longitude)
                <div id="location-map" style="height: 350px; border-radius: var(--radius-md); border: 2px solid var(--border-color);"></div>

                <div class="d-flex gap-4 mt-4" style="flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 200px; padding: 16px; background: var(--bg-tertiary); border-radius: var(--radius-md);">
                    ...
                    <div style="flex: 1; min-width: 200px; padding: 16px; background: var(--bg-tertiary); border-radius: var(--radius-md);">
                        <label class="text-muted" style="font-size: 12px; text-transform: uppercase;">Longitude</label>
                        <div class="fw-bold" style="font-family: monospace;">{{ $attendance->longitude }}</div>
                    </div>
                    @if($attendance->distance_meters)
                    <div style="flex: 1; min-width: 200px; padding: 16px; background: var(--bg-tertiary); border-radius: var(--radius-md);">
                        <label class="text-muted" style="font-size: 12px; text-transform: uppercase;">Jarak dari Kantor</label>
                        <div class="fw-bold" style="font-size: 18px;">
                            <span class="{{ $attendance->distance_meters <= 100 ? 'text-success' : 'text-warning' }}">
...
<script>
    ...
    L.polyline([[officeLat, officeLon], [checkInLat, checkInLon]], {
        color: '#6366f1',
        weight: 2,
        opacity: 0.7,
        dashArray: '5, 10'
