...
<div class="slide-up">
        ...
        <div class="d-flex justify-between align-center" style="flex-wrap: wrap; gap: 20px;">
            <div>
                ...
                <p class="text-muted" style="margin-top: 8px;">
                    <i class="fas fa-calendar"></i> Periode: {{ $intern->start_date->format('d M Y') }} - {{ $intern->end_date->format('d M Y') }}
                </p>
            </div>
    ...
    <div class="card mb-6">
        <div class="card-header border-0 pb-0">
             ...
             <small class="text-muted">Masuk ke dalam lingkaran merah untuk check-in (Max {{ $maxDist ?? 100 }}m).</small>
        </div>
        <div class="card-body">
            <div id="map"></div>
            <div class="d-flex justify-between align-center mt-3 p-3 bg-tertiary rounded" style="background: var(--bg-tertiary); border: 1px solid var(--border-color);">
                 <div>
    ...
    <div class="stat-grid">
        ...
        <div class="stat-card">
            ...
            <div class="progress mt-4">
                <div class="progress-bar" style="width: {{ $attendancePercentage }}%; background: linear-gradient(90deg, #22c55e, #16a34a);"></div>
            </div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div class="progress mt-4">
                <div class="progress-bar" style="width: {{ min($averageSpeed, 100) }}%; background: linear-gradient(90deg, #06b6d4, #0891b2);"></div>
            </div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div class="progress mt-4">
                <div class="progress-bar" style="width: {{ $overallScore }}%; background: linear-gradient(90deg, #f59e0b, #d97706);"></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid-2">
        ...
        <div class="card">
            ...
            @else
                ...
                <div class="task-item" style="padding: 16px; border: 1px solid var(--border-color); border-radius: var(--radius-md); margin-bottom: 12px; transition: all 0.3s;">
                    ...
                    <div class="d-flex justify-between align-center gap-2">
                        @if($task->status !== 'completed')
                            <form action="{{ route('tasks.updateStatus', $task) }}" method="POST" class="d-flex" style="flex: 1;">
                                @csrf
...
<style>
    ...
    .submission-stat-item.on-time { border-bottom: 4px solid var(--success); }
    .submission-stat-item.late { border-bottom: 4px solid var(--warning); }
    .submission-stat-item.pending { border-bottom: 4px solid var(--accent-primary); }
    .submission-stat-item.overdue { border-bottom: 4px solid var(--danger); }
    ...
    .submission-stat-item.on-time .stat-number { color: var(--success); }
    .submission-stat-item.late .stat-number { color: var(--warning); }
    .submission-stat-item.pending .stat-number { color: var(--accent-primary); }
    .submission-stat-item.overdue .stat-number { color: var(--danger); }
    ...
    .submission-stat-item.on-time .stat-icon-bg { color: var(--success); }
    .submission-stat-item.late .stat-icon-bg { color: var(--warning); }
    .submission-stat-item.pending .stat-icon-bg { color: var(--accent-primary); }
    .submission-stat-item.overdue .stat-icon-bg { color: var(--danger); }
</style>
@endpush
